...
type MessageMapper<T> = (raw: T) => Message;
...
type Options<TQueryRaw, TMutationRaw> = {
  ...
  /* queryFn이 반환한 채팅 내역의 각 요소를 Message로 변환하는 mapper */
  queryMessageMapper: MessageMapper<TQueryRaw>;

  /* 유저 입력(content)을 넘겨서 AI응답 1개를 받아오는 함수 */
  mutationFn: (content: string) => Promise<TMutationRaw>; 

  /* mutationFn의 raw 응답을 Message로 변환하는 mapper */
  mutationMessageMapper: MessageMapper<TMutationRaw>;
...
export default function useOptimisticChat<TQeuryRaw, TMutationRaw>({ 
  ...
  const { 
    ...
    queryFn: async () => {
      ...
      return rawList.map(queryMessageMapper);
    },
    staleTime,
    gcTime,
  });
  ...
  const mutation = useMutation<TMutationRaw, unknown, string>({
    ...
    onSuccess: (rawAiResponse) => { 
      // 서버의 응답을 Message로 변환
      const aiMessage = mutationMessageMapper(rawAiResponse);
